(function(){
  ...
  function renderMemory(){
    var q=trim(id('memorySearch').value).toLowerCase(),html='',i,a,w,j,word,reading,romaji,vi,en;
    for(i=0;i<appState.savedWords.length;i++){
      ...
      if(q && (word+' '+reading+' '+romaji+' '+vi+' '+en).toLowerCase().indexOf(q)<0)continue;
      html+='<div class="memory-item"><b>'+esc(word)+'</b>'+
          '<div class="small">'+esc(reading)+'</div>'+ '<div class="small">Phát âm: '+esc(romaji||'—')+'</div>'+
