(function(){
  ...
  function renderMemory(){
    ...
    for(i=0;i<appState.savedPhrases.length;i++){
      ...
      html+='<div class="memory-item saved-phrase">'+
          '<input class="memory-check" type="checkbox" data-memory-type="phrase" data-memory-index="'+i+'">'+
    ...
    for(i=0;i<appState.savedWords.length;i++){
      ...
      vi=w.meaningVi||'';
      en=w.meaningEn||w.meaning||w[1]||'';
      h=(word+' '+reading+' '+romaji+' '+vi+' '+en).toLowerCase();
      if(q&&h.indexOf(q)<0)continue;
      html+='<div class="memory-item saved-word">'+
          '<input class="memory-check" type="checkbox" data-memory-type="word" data-memory-index="'+i+'">'+
          '<div class="memory-check-body">'+
    ...
    for(i=0;i<appState.analyses.length;i++){
      ...
      if(q&&h.indexOf(q)<0)continue;
      html+='<div class="memory-item saved-analysis">'+
          '<input class="memory-check" type="checkbox" data-memory-type="analysis" data-memory-index="'+i+'">'+
          '<div class="memory-check-body">'+
          '<b class="memory-jp">'+esc(a.source||'')+'</b>'+
          ...
          '<div class="memory-actions"><button data-read-analysis="'+i+'">▶ Đọc</button><button data-open-analysis="'+i+'">Mở lại phân tích</button></div>'+
          '</div>'+
          '</div>';
    }
  ...
  function memoryChecks(){
    return qsa('#memoryList .memory-check');
  }

  function updateMemorySelectedCount(){
    var checks=memoryChecks(),i,count=0,parent;
    for(i=0;i<checks.length;i++){
      parent=checks[i].parentNode;
      if(checks[i].checked){
        count++;
        addClass(parent,'memory-item-selected');
      }else{
        removeClass(parent,'memory-item-selected');
      }
    }
    if(id('memorySelectedCount'))id('memorySelectedCount').innerHTML=count+' đã chọn';
  }

  function setAllMemoryChecks(value){
    var checks=memoryChecks(),i;
    for(i=0;i<checks.length;i++)checks[i].checked=value;
    updateMemorySelectedCount();
  }

  function deleteSelectedMemory(){
    var checks=memoryChecks(),i,type,index;
    var phraseIndexes=[],wordIndexes=[],analysisIndexes=[];

    for(i=0;i<checks.length;i++){
      if(!checks[i].checked)continue;
      type=checks[i].getAttribute('data-memory-type');
      index=parseInt(checks[i].getAttribute('data-memory-index'),10);
      if(isNaN(index))continue;

      if(type==='phrase')phraseIndexes.push(index);
      else if(type==='word')wordIndexes.push(index);
      else if(type==='analysis')analysisIndexes.push(index);
    }

    var total=phraseIndexes.length+wordIndexes.length+analysisIndexes.length;
    if(!total){
      toast('Chưa chọn mục để xóa.');
      return;
    }

    if(!window.confirm('Xóa '+total+' mục đã chọn khỏi Từ đã lưu?'))return;

    phraseIndexes.sort(function(a,b){return b-a;});
    wordIndexes.sort(function(a,b){return b-a;});
    analysisIndexes.sort(function(a,b){return b-a;});

    for(i=0;i<phraseIndexes.length;i++){
      if(phraseIndexes[i]>=0&&phraseIndexes[i]<appState.savedPhrases.length)appState.savedPhrases.splice(phraseIndexes[i],1);
    }
    for(i=0;i<wordIndexes.length;i++){
      if(wordIndexes[i]>=0&&wordIndexes[i]<appState.savedWords.length)appState.savedWords.splice(wordIndexes[i],1);
    }
    for(i=0;i<analysisIndexes.length;i++){
      if(analysisIndexes[i]>=0&&analysisIndexes[i]<appState.analyses.length)appState.analyses.splice(analysisIndexes[i],1);
    }

    /*
     * Save once, then render once. Avoid repeated localStorage writes / DOM rebuilds
     * on old iPad Safari.
     */
    try{
      localStorage.setItem(STORAGE_KEY,JSON.stringify(appState));
      ...
      toast('Không lưu được thay đổi.');
    }

    renderMemory();
    toast('Đã xóa '+total+' mục.');
  }
  ...
  function bind(){
    ...
    id('memorySelectAllBtn').onclick=function(){setAllMemoryChecks(true);};
    id('memoryClearSelectBtn').onclick=function(){setAllMemoryChecks(false);};
    id('memoryDeleteSelectedBtn').onclick=deleteSelectedMemory;
    id('memoryList').onclick=function(e){
      ...
      if(t.className&&String(t.className).indexOf('memory-check')>=0){
        updateMemorySelectedCount();
        return;
      }
      ...
      if(n!==null&&n!==''){
        var w=appState.savedWords[parseInt(n,10)];
        if(w)speakJapaneseText(w.reading||w.word||'');
        return;
      }

      n=t.getAttribute('data-read-phrase');
      if(n!==null&&n!==''){
        var p=appState.savedPhrases[parseInt(n,10)];
        if(p)speakJapaneseText(p.source||p.hiragana||'');
        return;
      }

      n=t.getAttribute('data-open-phrase');
      if(n!==null&&n!==''){
        openSavedPhrase(parseInt(n,10));
        return;
      }

      n=t.getAttribute('data-read-analysis');
      if(n!==null&&n!==''){
        var a=appState.analyses[parseInt(n,10)];
        if(a)speakJapaneseText(a.source||analysisReading(a)||'');
        return;
      }

      n=t.getAttribute('data-open-analysis');
      if(n!==null&&n!==''){
        openSavedAnalysis(parseInt(n,10));
        return;
      }
    };
