(function(){
  ...
  var wheelLock=false,touchX=0,touchY=0,videoBadge;
  ...
  mediaPanel.addEventListener('touchstart',function(event){if(event.touches.length!==1)return;touchX=event.touches[0].clientX;touchY=event.touches[0].clientY},{passive:true});
  mediaPanel.addEventListener('touchend',function(event){var t=event.changedTouches[0],dx=t.clientX-touchX,dy=t.clientY-touchY;if(Math.abs(dy)>70&&Math.abs(dy)>Math.abs(dx)*1.18){pauseMedia();move(dy<0?1:-1)}},{passive:true});
