...
$('uploadBtn').onclick=()=>$('file').click();$('file').onchange=e=>extract(e.target.files[0]);

$('urlBtn').onclick=async()=>{
 const resourceUrl=$('url').value.trim();if(!resourceUrl)return;
 setStatus('Đang tải PDF từ URL...');
 try{
   const r=await fetch('/api/extract-text/url',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({resourceUrl,language:'vie'})});
   const data=await r.json().catch(()=>({}));if(!r.ok||data.status!=='success')throw new Error(data.error||'Không thể tải URL');
   const extracted=(data.pages||[]).map((p,i)=>({pageNumber:Number(p.pageNumber)||i+1,text:String(p.text||'')}));
...
$('login').onclick=()=>{document.cookie='PORTAL_LOGIN_RETURN='+encodeURIComponent(location.pathname+location.search+location.hash)+'; Max-Age=600; Path=/; SameSite=Lax'};
