	  ...
	  if(document.querySelector("#process")){
		  document.querySelector("#process").addEventListener("click", function (event) {
					...
					let email = document.getElementById("c_email_address");
					    ...
					    if (carrierVal === 'viettel') {
					        ...
					        if (!vpPhone || !validatePhoneNumber(vpPhone.value)) {
						        ...
						        const ghtkWarehouse = document.querySelector('input[name="selectedWarehouse"]:checked');
						        const ghtkProvince = document.getElementById('ghtk_receiver_province');
						        const ghtkDistrict = document.getElementById('ghtk_receiver_district');
						        const ghtkStreet = document.getElementById('ghtk_receiver_street');
						        let ghtkPhone = document.getElementById('ghtk_receiver_phone');
						        if (!ghtkWarehouse) {
						            isValid = false;
						            showError(button, " Vui long chon kho gui hang GHTK");
						        }
						        if (!ghtkProvince || !ghtkProvince.value.trim()) {
						            isValid = false;
						            if (ghtkProvince) showError(ghtkProvince, " Vui long nhap tinh/thanh nhan hang");
						        }
						        if (!ghtkDistrict || !ghtkDistrict.value.trim()) {
						            isValid = false;
						            if (ghtkDistrict) showError(ghtkDistrict, " Vui long nhap quan/huyen nhan hang");
						        }
						        if (!ghtkStreet || !ghtkStreet.value.trim()) {
						            isValid = false;
						            if (ghtkStreet) showError(ghtkStreet, " Vui long nhap dia chi nhan hang");
						        }
						        if (!ghtkPhone || !validatePhoneNumber(ghtkPhone.value)) {
						            isValid = false;
						            if (ghtkPhone) showError(ghtkPhone, " SDT nhan hang khong hop le");
						        }
                            } else if (carrierVal === 'spx') {
                                const spxWarehouse = document.querySelector('input[name="selectedWarehouse"]:checked');
                                const spxPhone = document.getElementById("c_phone");
                                const spxProvince = document.getElementById("c_state_country");
                                const spxDistrict = document.getElementById("c_district");
                                const spxStreet = document.getElementById("c_address");
                                const spxWeight = parseInt(document.getElementById('spx_product_weight')?.value || 0);
                                if (!spxWarehouse) {
                                    isValid = false;
                                    showError(button, " Vui long chon kho gui hang SPX");
                                }
                                if (!spxProvince || !spxProvince.value.trim()) {
                                    isValid = false;
                                    if (spxProvince) showError(spxProvince, " Vui long nhap tinh/thanh nhan hang");
                                }
                                if (!spxDistrict || !spxDistrict.value.trim()) {
                                    isValid = false;
                                    if (spxDistrict) showError(spxDistrict, " Vui long nhap quan/huyen nhan hang");
                                }
                                if (!spxStreet || !spxStreet.value.trim()) {
                                    isValid = false;
                                    if (spxStreet) showError(spxStreet, " Vui long nhap dia chi nhan hang");
                                }
                                if (!spxPhone || !validatePhoneNumber(spxPhone.value)) {
                                    isValid = false;
                                    if (spxPhone) showError(spxPhone, " SDT nhan hang khong hop le");
                                }
                                if (!spxWeight || spxWeight <= 0) {
                                    isValid = false;
                                    const spxWeightEl = document.getElementById('spx_product_weight');
                                    if (spxWeightEl) showError(spxWeightEl, " Vui long nhap trong luong SPX");
                                }
                                const spxOption = document.querySelector('input[name="spxShippingOption"]:checked');
                                const spxPrice = parseInt(spxOption?.getAttribute('data-price') || 0);
                                if (!spxOption || spxPrice <= 0) {
                                    isValid = false;
                                    showError(button, " Vui long tinh phi SPX truoc khi dat hang");
                                }
                            } else {
			        ...
			        if (!isValid) {
						...
						if (carrier === 'viettel') {
							...
							if (selOpt) {
								...
								const ghtkWardEl = document.getElementById('ghtk_receiver_ward');
								const ghtkStreetEl = document.getElementById('ghtk_receiver_street');
								const ghtkPhoneEl = document.getElementById('ghtk_receiver_phone');
								const ghtkFallback = (typeof window !== 'undefined' && window.userDetails) ? window.userDetails : {};
								const ghtkReceiverProvince = (ghtkProvSel?.value || '').trim();
								const ghtkReceiverDistrict = (ghtkDistSel?.value || '').trim();
								const ghtkReceiverWard = (ghtkWardEl?.value || '').trim();
								const ghtkReceiverStreet = (ghtkStreetEl?.value || '').trim();
								const ghtkReceiverPhone = (ghtkPhoneEl?.value || '').trim();
								const ghtkPickProvince = (document.getElementById('ghtk_sender_province')?.value || ghtkFallback.ghtkPickProvince || '').trim();
								const ghtkPickDistrict = (document.getElementById('ghtk_sender_district')?.value || ghtkFallback.ghtkPickDistrict || '').trim();
								userDetails['city']        = ghtkReceiverProvince;
								userDetails['district']    = ghtkReceiverDistrict;
								userDetails['address']     = ghtkReceiverStreet;
								userDetails['zipcode']     = '';
								userDetails['phoneNumber'] = ghtkReceiverPhone;
								userDetails['selectedAddress'] = '';
								userDetails['ghtkPickProvince']     = ghtkPickProvince;
								userDetails['ghtkPickDistrict']     = ghtkPickDistrict;
								userDetails['ghtkReceiverProvince'] = ghtkReceiverProvince;
								userDetails['ghtkReceiverDistrict'] = ghtkReceiverDistrict;
								userDetails['ghtkReceiverWard']     = ghtkReceiverWard;
								userDetails['ghtkReceiverStreet']   = ghtkReceiverStreet;
								userDetails['ghtkReceiverPhone']    = ghtkReceiverPhone;
								...
								if (ghtkWhRadio) {
									...
									userDetails['warehouseId']   = ghtkFallback.warehouseId || userDetails['warehouseId'] || '';
									userDetails['warehouseName'] = ghtkFallback.warehouseName || userDetails['warehouseName'] || '';
								}
                                ...
                                const spxAddressEl = document.getElementById("c_address");
                                const spxDistrictEl = document.getElementById("c_district");
                                const spxCityEl = document.getElementById("c_state_country");
                                const spxPhoneEl = document.getElementById("c_phone");
                                const spxZipEl = document.getElementById("c_postal_zip");
                                const spxSelectedAddressEl = document.getElementById("selectedAddressId");
                                userDetails['address']  = (spxAddressEl?.value || '').trim();
                                userDetails['district'] = (spxDistrictEl?.value || '').trim();
                                userDetails['city']     = (spxCityEl?.value || '').trim();
                                userDetails['phoneNumber'] = (spxPhoneEl?.value || '').trim();
                                userDetails['selectedAddress'] = spxSelectedAddressEl?.value || '';
                                userDetails['zipcode']  = (spxZipEl?.value || '').trim();
                                const spxWhRadio = document.querySelector('input[name="selectedWarehouse"]:checked');
                                if (spxWhRadio) {
                                    userDetails['warehouseId']   = spxWhRadio.value;
                                    userDetails['warehouseName'] = spxWhRadio.getAttribute('data-wh-name') || '';
                                }
                                userDetails['weight']    = parseInt(document.getElementById('spx_product_weight')?.value || 0);
                                userDetails['codAmount'] = parseInt(document.getElementById('spx_money_collection')?.value || 0);
                                const spxSelOpt = document.querySelector('input[name="spxShippingOption"]:checked');
                                const spxCode = spxSelOpt?.getAttribute('data-code') || spxSelOpt?.value || 'spx';
                                const spxName = spxSelOpt?.getAttribute('data-name') || 'SPX Shopee Express';
                                const spxPrice = parseInt(spxSelOpt?.getAttribute('data-price') || 0);
                                userDetails['viettelTypeCode']      = spxCode;
                                userDetails['viettelTypeName']      = spxName;
                                userDetails['viettelShippingPrice'] = spxPrice;
							} else {
							...
							// Snapshot carrier fields that /api/createuser can drop from checkout-only transient data.
							// Keep selectedAddress from the backend response because it is the persisted shipping row id.
							const _shippingSnap = {};
							if (carrier !== 'standard') {
								['shippingAgent','city','district','address','phoneNumber','zipcode',
								 ...
								 'vpSenderProvinceId','vpSenderDistrictId','vpReceiverProvinceId','vpReceiverDistrictId',
								 'viettelReceiverProvince','viettelReceiverDistrict','viettelReceiverWard','viettelReceiverStreet','viettelReceiverPhone'
								].forEach(k => { _shippingSnap[k] = userDetails[k]; });
							}
						...
						.then(userClient => {
								console.log('User data received:', userClient);
								const savedSelectedAddress = userClient && userClient.selectedAddress ? userClient.selectedAddress : '';
								userDetails= userClient;
									if (_shippingSnap.shippingAgent) { Object.assign(userDetails, _shippingSnap); }
								if (savedSelectedAddress) { userDetails['selectedAddress'] = savedSelectedAddress; }
		    ...
		    function showError(input, message) {
		        ...
		        if (!errorDiv) {
		            errorDiv = input.parentElement.querySelector(".text-danger");
		        }
		        if (!errorDiv) {
