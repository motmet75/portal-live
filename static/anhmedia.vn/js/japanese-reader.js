(() => {
  ...
  function openBookmark(documentId, bookmarkId) { const doc = state.documents.find(item => item.id === documentId); const mark = doc && normalizedBookmarks(doc).find(item => item.id === bookmarkId); if (!doc || !mark) return; currentDocumentId = doc.id; currentPages = doc.pages || [doc.html || '<p></p>']; $('[data-document-title]').value = doc.title; renderPage(mark.page, false); requestAnimationFrame(() => { let target = mark.anchorId ? editor.querySelector(`[data-jp-bookmark-anchor="${mark.anchorId}"]`) : null; if (!target && mark.excerpt) target = [...editor.querySelectorAll('p,li,h1,h2,h3,div')].find(el => el.textContent.includes(mark.excerpt.slice(0, 50))); target = target || editor; target.scrollIntoView({ behavior: 'smooth', block: 'center' }); target.classList.add('jp-bookmark-focus'); setTimeout(() => target.classList.remove('jp-bookmark-focus'), 2200); }); toast(`Trang ${mark.page + 1}: ${mark.note}`); }
  ...
  $('[data-memory-search]').addEventListener('input', event => { memorySearchTerm = event.currentTarget.value.trim(); renderMemory(); });
