(() => {
  ...
  function renderDocuments() { $('[data-document-list]').innerHTML = state.documents.slice(0, 8).map(item => { const bookmarks = item.bookmarks || []; return `<article class="jp-document-row"><button class="jp-document" type="button" data-document-id="${item.id}"><strong>${escapeHtml(item.title)}</strong><small>${(item.pages || [item.html]).length} trang · ${bookmarks.length} dấu trang · ${new Date(item.updatedAt).toLocaleDateString('vi-VN')}</small></button><button class="jp-document-delete" type="button" data-delete-document="${item.id}" aria-label="Xóa ${escapeHtml(item.title)}" title="Xóa tài liệu">×</button>${bookmarks.length ? `<nav class="jp-bookmark-list" aria-label="Dấu trang">${bookmarks.sort((a, b) => a - b).map(page => `<button type="button" data-bookmark-document="${item.id}" data-bookmark-page="${page}">★ Trang ${page + 1}</button>`).join('')}</nav>` : ''}</article>`; }).join(''); }
  function toggleBookmark() { const doc = state.documents.find(item => item.id === currentDocumentId); if (!doc) { toast('Hãy tải lên hoặc lưu tài liệu trước khi đánh dấu trang.'); return; } doc.bookmarks = doc.bookmarks || []; const position = doc.bookmarks.indexOf(currentPageIndex); if (position >= 0) doc.bookmarks.splice(position, 1); else doc.bookmarks.push(currentPageIndex); doc.currentPage = currentPageIndex; doc.updatedAt = new Date().toISOString(); persist(); renderPage(currentPageIndex, false); toast(position >= 0 ? 'Đã bỏ dấu trang.' : `Đã đánh dấu trang ${currentPageIndex + 1}.`); }
  ...
  $('[data-document-list]').addEventListener('click', event => { const bookmark = event.target.closest('[data-bookmark-page]'); if (bookmark) { const doc = state.documents.find(item => item.id === Number(bookmark.dataset.bookmarkDocument)); if (doc) { currentDocumentId = doc.id; currentPages = doc.pages || [doc.html || '<p></p>']; $('[data-document-title]').value = doc.title; renderPage(Number(bookmark.dataset.bookmarkPage), false); } return; } const remove = event.target.closest('[data-delete-document]'); if (remove) { deleteDocument(Number(remove.dataset.deleteDocument)); return; } const button = event.target.closest('[data-document-id]'); if (!button) return; const doc = state.documents.find(item => item.id === Number(button.dataset.documentId)); if (doc) { currentDocumentId = doc.id; currentPages = doc.pages || [doc.html || '<p></p>']; $('[data-document-title]').value = doc.title; renderPage(doc.currentPage || 0, false); } });
  ...
  $$('[data-page-go]').forEach(button => button.addEventListener('click', () => goToPage(button.closest('[data-page-nav]').querySelector('[data-page-input]').value))); $$('[data-page-input]').forEach(input => input.addEventListener('keydown', event => { if (event.key === 'Enter') goToPage(event.currentTarget.value); })); $$('[data-page-bookmark]').forEach(button => button.addEventListener('click', toggleBookmark));
