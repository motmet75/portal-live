(() => {
  ...
  function reopenSession(sessionId) { const analysis = state.analyses.find(item => String(item.sessionId) === String(sessionId)); if (!analysis) { toast('Không tìm thấy phiên học đã lưu.'); return; } const doc = state.documents.find(item => String(item.id) === String(analysis.documentId)); if (doc) { currentDocumentId = doc.id; currentPages = doc.pages || [doc.html || '<p></p>']; $('[data-document-title]').value = doc.title; renderPage(analysis.pageIndex || 0, false); } currentAnalysis = { ...analysis }; hasUnsavedAnalysis = false; selectedText = analysis.source || ''; setView(false); renderAnalysis(); toast(doc ? `Đã trở lại trang ${(analysis.pageIndex || 0) + 1}.` : 'Đã mở phiên học; chưa tìm thấy tài liệu gốc.'); }
