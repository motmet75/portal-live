(() => {
  ...
  function renderPage(index, saveCurrent = true) { if (saveCurrent) storeCurrentPage(); currentPageIndex = Math.max(0, Math.min(index, currentPages.length - 1)); bookmarkExcerpt = ''; editor.innerHTML = currentPages[currentPageIndex] || '<p></p>'; const doc = state.documents.find(item => item.id === currentDocumentId); const pageBookmarkCount = doc ? normalizedBookmarks(doc).filter(item => item.page === currentPageIndex).length : 0; $$('[data-page-label]').forEach(el => { el.textContent = `Trang ${currentPageIndex + 1} / ${currentPages.length}`; }); $$('[data-page-input]').forEach(el => { el.value = currentPageIndex + 1; el.max = currentPages.length; }); $$('[data-page-prev]').forEach(el => { el.disabled = currentPageIndex === 0; }); $$('[data-page-next]').forEach(el => { el.disabled = currentPageIndex >= currentPages.length - 1; }); $$('[data-page-bookmark]').forEach(el => { el.disabled = !doc; el.classList.toggle('is-active', pageBookmarkCount > 0); el.textContent = pageBookmarkCount ? `★ Lưu dấu (${pageBookmarkCount})` : '☆ Lưu dấu'; }); $('[data-document-meta]').textContent = `TRANG ${currentPageIndex + 1} / ${currentPages.length} · Chọn đoạn ngắn để học`; if (doc) { doc.currentPage = currentPageIndex; localStorage.setItem(storageKey, JSON.stringify(state)); } }
  ...
  function renderDocuments() { $('[data-document-list]').innerHTML = state.documents.slice(0, 8).map(item => { const bookmarks = normalizedBookmarks(item); return `<article class="jp-document-row"><button class="jp-document" type="button" data-document-id="${item.id}"><strong>${escapeHtml(item.title)}</strong><small>${(item.pages || [item.html]).length} trang · ${bookmarks.length} dấu đoạn · ${new Date(item.updatedAt).toLocaleDateString('vi-VN')}</small></button><button class="jp-document-delete" type="button" data-delete-document="${item.id}" aria-label="Xóa ${escapeHtml(item.title)}" title="Xóa tài liệu">×</button>${bookmarks.length ? `<div class="jp-bookmark-list" aria-label="Dấu trang">${[...bookmarks].sort((a, b) => a.page - b.page).map(mark => `<article><button class="jp-bookmark-open" type="button" data-bookmark-document="${item.id}" data-bookmark-page="${mark.page}"><b>★ Trang ${mark.page + 1}</b><span>${escapeHtml(mark.note || 'Không có ghi chú')}</span>${mark.excerpt ? `<small>${escapeHtml(mark.excerpt)}</small>` : ''}</button><button class="jp-bookmark-delete" type="button" data-delete-bookmark="${escapeHtml(mark.id)}" data-bookmark-document="${item.id}" aria-label="Xóa dấu trang">×</button></article>`).join('')}</div>` : ''}</article>`; }).join(''); }
  function addBookmark(note) { const doc = state.documents.find(item => item.id === currentDocumentId); if (!doc) { toast('Hãy tải lên hoặc lưu tài liệu trước khi đánh dấu đoạn.'); return; } const cleanNote = String(note || '').trim(); if (!cleanNote) { toast('Nhập ghi chú để nhớ lý do đánh dấu đoạn này.'); return; } const excerpt = bookmarkExcerpt || editor.querySelector('p')?.innerText?.trim().slice(0, 300) || editor.innerText.trim().slice(0, 300); normalizedBookmarks(doc).push({ id: `mark-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, page: currentPageIndex, note: cleanNote.slice(0, 240), excerpt, savedAt: new Date().toISOString() }); doc.currentPage = currentPageIndex; doc.updatedAt = new Date().toISOString(); $$('[data-bookmark-note]').forEach(input => { input.value = ''; }); persist(); renderPage(currentPageIndex, false); toast(`Đã lưu ghi chú tại trang ${currentPageIndex + 1}.`); }
  function deleteBookmark(documentId, bookmarkId) { const doc = state.documents.find(item => item.id === documentId); if (!doc) return; doc.bookmarks = normalizedBookmarks(doc).filter(item => item.id !== bookmarkId); persist(); if (currentDocumentId === documentId) renderPage(currentPageIndex, false); toast('Đã xóa dấu đoạn.'); }
  ...
  $('[data-document-list]').addEventListener('click', event => { const removeBookmark = event.target.closest('[data-delete-bookmark]'); if (removeBookmark) { deleteBookmark(Number(removeBookmark.dataset.bookmarkDocument), removeBookmark.dataset.deleteBookmark); return; } const bookmark = event.target.closest('[data-bookmark-page]'); if (bookmark) { const doc = state.documents.find(item => item.id === Number(bookmark.dataset.bookmarkDocument)); if (doc) { currentDocumentId = doc.id; currentPages = doc.pages || [doc.html || '<p></p>']; $('[data-document-title]').value = doc.title; renderPage(Number(bookmark.dataset.bookmarkPage), false); } return; } const remove = event.target.closest('[data-delete-document]'); if (remove) { deleteDocument(Number(remove.dataset.deleteDocument)); return; } const button = event.target.closest('[data-document-id]'); if (!button) return; const doc = state.documents.find(item => item.id === Number(button.dataset.documentId)); if (doc) { currentDocumentId = doc.id; currentPages = doc.pages || [doc.html || '<p></p>']; $('[data-document-title]').value = doc.title; renderPage(doc.currentPage || 0, false); } });
