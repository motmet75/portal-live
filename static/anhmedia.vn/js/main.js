   ...
   function displayResults(items, hasMore, catalogCode, searchTerm ) {
     ...
     if (hasMore) {
       ...
       viewAllItem.innerHTML = `
         <a href="/trang-chu.html?catalog=${encodeURIComponent(catalogCode)}&keyword=${encodeURIComponent(searchTerm)}">
...
function newFunction(priceElements) {
    priceElements.forEach((priceElement) => {
		...
		if((priceText.includes('đ') || priceText.toLowerCase().includes('vn'))){
	        if (priceText.includes(' - ')) {
	            ...
	            const formattedPrices = prices.map((price) => {
					...
					if(numericValue.length > 3){
						...
						return `<span class="">${numericValue}</span>`;
					}
	            });
				...
				if (numericValue.length > 3) {
					priceElement.innerHTML = `${mainPart}<span class="smaller-digits">${lastThreeDigits}đ</span>`;
				} else {
					priceElement.innerHTML = `${mainPart}<span class="">${lastThreeDigits}đ</span>`;
				}
     ...
     function submitSizeForm(event,button) {
		...
		const form = button.form; // Use the clicked option button, even when a child image receives the tap
		if (!form) {
			return;
		}
