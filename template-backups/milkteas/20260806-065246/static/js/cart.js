    ...
    input.addEventListener('change', (event) => {
        ...
        if (button) {
			...
			if (usernameElement && emailElement) {
					...
					userClient.firstName = usernameElement.value;
					userClient.lastName = emailElement.value;
			 }
...
function updateCartUIPage() {
    ...
    cart.items.forEach(item => {
		...
		<td class="product-thumbnail">
											...
											<img src="${item.imageUrl}" alt="Image" class="img-fluid" style="width:100%">
											</td>
											<td class="product-name">
												
												<a th:href="@{/product/{id}(id=${item.path})}">
																<h2 class="h5 text-black" >${item.name}</h2>
												</a>
											</td>
											<td class="product-name">
												<h2 class="h5 text-black" >${item.colorCode} : ${item.sizeCode}</h2>
											</td>
											<td class="product-price small"  style="font-weight:bold;color:red;  font-size:18px;">${item.formatedPrice}</td>
											...
											<td class="product-price small"  style="font-weight:bold;color:red;  font-size:18px;">${item.totalPrice}${cart.currency}</td>
